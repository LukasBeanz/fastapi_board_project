...
from datetime import datetime, timezone  # 게시글 작성일과 수정일의 기본값을 만들기 위해 사용합니다.
...
class User(Base):
    ...
    created_at = Column(DateTime, default=utcnow, nullable=False)  # 회원 가입 시각을 UTC 기준으로 저장합니다.
...
class Board(Base):
    ...
    created_at = Column(DateTime, default=utcnow, nullable=False)  # 게시글 작성 시각입니다.
    updated_at = Column(DateTime, default=utcnow, onupdate=utcnow, nullable=False)  # 수정 시 자동 갱신되는 시각입니다.
